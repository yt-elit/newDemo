...
function get_list() {
    ...
    $.getJSON(ApiUrl + '/index.php?act=voucher&op=voucher_tpl_list' + window.location.search.replace('?','&'), param, function(result){
    	if(!result ) {
    		result = [];
    		result.datas = [];
    		result.datas.voucher_list = [];
    	}
        $('.loading').remove();
        curpage++;
...
function getVoucher(dom){
	var vid	=	dom.getAttribute('goods_id');
	getFreeVoucher(vid);
}
