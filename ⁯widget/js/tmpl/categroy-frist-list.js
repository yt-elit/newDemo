$(function() {
    ...
    $("#header").on('click', '.header-inp', function(){
    	if( sysType == 'wx' ){
    		...
    		k('tmpl/search.html','sousuoshangpin')
    	}
    });
    $.getJSON(ApiUrl+"/index.php?act=goods_class&gc_id=1", function(result){
	...
	$('#categroy-cnt').on('click','.category', function(){
	    ...
	    $.getJSON(ApiUrl + '/index.php?act=goods_class', {gc_id:gc_id}, function(result){
	        var data = result.datas;
            data.WapSiteUrl = WapSiteUrl;
