...
var tianjiazuiduoshu = 1;//图片最多上传数量
...
function fabu(){
	...
	var shuju	=	{'circle_name':neirong,'circle_id':id,'images':images,'video':yuancheng_video,'type':2};
	var dizhi	=	ApiUrl + '/index.php?act=circle&op=etidAjax';
	var func	=	'_handle2';
	var op		=	{'dizhi':dizhi,'func':func,'shuju':shuju};
...
function shangchuanwenjian(){
	if( !is_uploading ){//第一次进来,组建数据
		...
		shangchuanshipin();//图片上传完毕,正式发布二手
	}
}
 ...
 * 上传视频
 */
function shangchuanshipin(){
	if( bendi_video != '' && yuancheng_video != '' ){
		fabu();
	}else{
		api.ajax({
		    url: apiUrl + '/Home/Public/Upload',
		    ...
		    data: {
				...
				die(ret);
				if( ret.progress == 100 && ret.status == 1  ){
					yuancheng_video = ret.body.listData.img;
					fabu();
				}
		});
	}
...
function dakaiziyuan(){
	...
	var op = {};
		op.name = 'luxiang-w';
		op.html = '../duanshipin/luxiang-w.html';
	ow(op);
}
...
function yulantupian(){
	$('.img-list .suolvtu').off('click');
	$('.img-list .suolvtu').on('click',function(){
		var ii = $('.img-list .suolvtu').index(this);
		
		var img_arr = [];
		$('.img-list .suolvtu').each(function(e,i){
			var path = $(this).attr('src');
			img_arr.push( path );
		})
		msxDakaishipin(bendi_video);
	})
}
